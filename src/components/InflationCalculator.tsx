...
const InflationCalculator = () => {
  ...
  const calculateInflation = () => {
    const amount = parseFloat(presentValue);
    ...
    // Calculate purchasing power (how much today's money will be worth in the future)
    ...
    for (let year = 0; year <= time; year++) {
      const yearFutureValue = amount * Math.pow(1 + rate, year);
      const yearPurchasingPower = amount / Math.pow(1 + rate, year);
      data.push({
        year: year,
        futureValue: Number(yearFutureValue.toFixed(2)),
        purchasingPower: Number(yearPurchasingPower.toFixed(2))
      });
    }
    setChartData(data);
  };
  ...
  return (
    <div className="rounded-3xl bg-black/90 backdrop-blur-lg p-6 shadow-calc transition-all duration-500 hover:scale-[1.02] w-full max-w-[800px] text-white">
      <div className="flex items-center gap-2 mb-4">
        <TrendingUp size={20} />
        <h3 className="text-xl font-medium">Inflation Calculator</h3>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          ...
          <div>
            <label className="text-sm text-white/70 mb-1 block">Inflation Rate (% per year)</label>
            <input 
              type="number" 
              value={inflationRate}
          ...
          {result && (
            <div className="mt-4 p-4 bg-white/10 rounded-lg space-y-2">
              ...
              <div className="flex justify-between">
                <span className="text-white/70">Purchasing Power:</span>
                <span className="font-medium">${result.purchasingPower}</span>
              </div>
            </div>
          )}
        </div>
